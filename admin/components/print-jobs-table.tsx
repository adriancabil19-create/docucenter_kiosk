...
export function PrintJobsTable({ initialData }: Props) {
  ...
  const refresh = useCallback(async (silent = false) => {
    ...
    try {
      ...
      setRows(res.jobs);
      if (!silent) addToast({ title: 'Refreshed', description: `${res.count} print job(s) loaded.`, color: 'success' });
    } catch (err) {
      if (!silent) addToast({ title: 'Refresh failed', description: err instanceof Error ? err.message : 'Could not reach the server.', color: 'danger' });
    } finally {
